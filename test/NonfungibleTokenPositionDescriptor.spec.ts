...
describe('NonfungibleTokenPositionDescriptor', () => {
  ...
  describe('#tokenURI', () => {
    it('displays IP as token symbol for WIP token', async () => {
      ...
      expect(metadata.name).to.match(/(\sIP\/TEST|TEST\/IP)/)
      expect(metadata.description).to.match(/(TEST-IP|\sIP-TEST)/)
      expect(metadata.description).to.match(/(\nIP\sAddress)/)
    })

    it('displays returned token symbols when neither token is WIP ', async () => {
