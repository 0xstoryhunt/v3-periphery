...
import { IStoryHuntV3Pool, IWIP9, MockTimeSwapRouter, TestERC20 } from '../typechain'
...
describe('SwapRouter gas tests', function () {
  ...
  const swapRouterFixture: Fixture<{
    ...
    pools: [IStoryHuntV3Pool, IStoryHuntV3Pool, IStoryHuntV3Pool]
    ...
    const pools = poolAddresses.map((poolAddress) => new ethers.Contract(poolAddress, IStoryHuntV3PoolABI, wallet)) as [
      IStoryHuntV3Pool,
      IStoryHuntV3Pool,
      IStoryHuntV3Pool
    ]
  ...
  let pools: [IStoryHuntV3Pool, IStoryHuntV3Pool, IStoryHuntV3Pool]
  ...
  async function exactInput(
    ...
    const params = {
      ...
      deadline: 1,
      amountIn,
      amountOutMinimum: outputIsWIP9 ? 0 : amountOutMinimum, // save on calldata,
    }

    const data = [router.interface.encodeFunctionData('exactInput', [params])]
    if (outputIsWIP9) data.push(router.interface.encodeFunctionData('unwrapWIP9', [amountOutMinimum, trader.address]))
  ...
  describe('#exactInput', () => {
    ...
    it('0 -> 1 minimal', async () => {
      const calleeFactory = await ethers.getContractFactory('TestStoryHuntV3Callee')
